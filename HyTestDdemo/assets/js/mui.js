...
(function($, window, document, name) {
	...
	var onPopoverHidden = function(e) {
		...
		// 关闭后还原到原先的parentNode中, zhaohuihua, 2016-11-12
		if (this._parentNode && this.parentNode != this._parentNode) {
			this._parentNode.appendChild(this);
			this._parentNode = null;
		}
		var anchor = this._anchor;
		var cls = this._openedClassName;
		if (anchor && cls) { anchor.classList.remove(cls); }
		$.trigger(this, 'hidden', this);
	};

	var getBackdrop = function(document) {
		if (document._backdrop) { return document._backdrop; }
		...
		document._backdrop = element;
		return element;
	};
	var removeBackdropTimer;
	var removeBackdrop = function(popover) {
		// 直接用document在iframe中触发关闭时有问题, zhaohuihua, 2016-11-22
		var document = popover.ownerDocument;
		var backdrop = getBackdrop(document);
	...
	var togglePopover = function(popover, anchor, state, follow, cls) {
		...
		popover._openedClassName = cls;
		if (anchor && cls) { anchor.classList.add(cls); }
		// 如果不是body的直接子元素, 移到body中, zhaohuihua, 2016-11-12
		if (popover.parentNode != document.body) {
			popover._parentNode = popover.parentNode;
		...
		calPosition(popover, anchor, isActionSheet, follow); //position
		backdrop.classList.add(CLASS_ACTIVE);
		popover.addEventListener('webkitTransitionEnd', onPopoverShown);
	};
	var setStyle = function(popover, display, top, left, right) {
	...
	var calPosition = function(popover, anchor, isActionSheet, follow) {
		if (!popover || !anchor) {
			return;
		}
		...
		// window.innerWidth 比 body.offsetWidth 多出滚动条的宽度, zhaohuihua, 2016-11-12
		var wWidth = document.body.offsetWidth; // window.innerWidth;
	...
	$.fn.popover = function() {
		...
		this.each(function() {
			...
			if (args[0] === 'show' || args[0] === 'hide' || args[0] === 'toggle') {
				togglePopover(this, args[1], args[0], args[2], args[3]);
			}
		});
	};
